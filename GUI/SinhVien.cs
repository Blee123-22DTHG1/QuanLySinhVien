...
namespace QuanLySinhVien.GUI
{
    public partial class SinhVien : Form
    {
        ...
        {
            string kitu = txttimsv.Text.Trim();

            if (string.IsNullOrEmpty(kitu))
            {
                MessageBox.Show("Vui lòng nhập từ khóa tìm kiếm.", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            // Gọi phương thức timSV từ BLL để lấy dữ liệu
            DataTable dt = bllSV.timSV(kitu);

            // Kiểm tra và hiển thị kết quả lên DataGridView
