...
namespace QuanLySinhVien.GUI
{
    public partial class DangNhap : Form
    {
        ...
        {
            DataTable dtDN = bllDangNhap.getData($"SELECT tenDangNhap, matKhau, hoTen, role FROM TaiKhoan WHERE tenDangNhap = '{txtTDN.Text.Trim()}' AND matKhau = '{txtMK.Text.Trim()}'");

            if (dtDN.Rows.Count > 0)
            {
                string role = dtDN.Rows[0]["role"].ToString();
                ...
                else if (role == "teacher")
                {
                    ...
                    DiemThi formDiemThi = new DiemThi(trangChu, role);  // Truyền role vào DiemThi
